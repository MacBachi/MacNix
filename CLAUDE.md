...
- New system-wide macOS default (Finder, Dock, etc.) → `darwin/macos.nix`. **Per-host Dock overrides** (e.g. workmac wants Edge next to Firefox) go in `per-host/<hostname>.nix` with `system.defaults.dock.persistent-apps = lib.mkForce [...]` — needs the full list because the option is type list and would otherwise concatenate.
...
- **1Password is intentionally NOT nix-managed** despite being load-bearing (git signing depends on `/Applications/1Password.app/Contents/MacOS/op-ssh-sign`). nixpkgs hash drift, brew cask version staleness and CDN installer-wrapper changes all turned out to be too unstable to script reliably. `darwin/preflight.nix` shows a bright red warning on every activation if the .app is missing, pointing at `https://1password.com/downloads/mac`. After the first manual install, 1Password's own auto-updater takes over.
